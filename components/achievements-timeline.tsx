    ...
    <section id="achievements" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      <div className="mb-10 flex items-end justify-between gap-6">
        <div>
          ...
          <h2 className="text-3xl font-semibold tracking-tight text-white sm:text-4xl">A timeline of proof.</h2>
        </div>
      ...
      <div className="relative">
        <div className="absolute left-4 top-0 h-full w-px bg-gradient-to-b from-white/20 via-white/10 to-transparent md:left-1/2" />
        <div className="grid gap-6">
          {achievements.map((item, idx) => (
            <div key={item.title} className={`relative grid gap-6 md:grid-cols-2 ${idx % 2 ? "md:[&>*:first-child]:order-2" : ""}`}>
              <div className="flex items-center md:justify-end">
                <div className="absolute left-0 h-3 w-3 rounded-full border border-white/25 bg-[#77a4ff] md:left-1/2 md:-translate-x-1/2" />
                <div className="ml-10 md:ml-0 md:max-w-md md:text-right">
                  <p className="text-xs uppercase tracking-[0.4em] text-white/35">{item.year} • {item.level}</p>
